...
2. `data/product.db`
   - 产品数据仓库，由离线数据脚本生成。
   - 存放市场榜单、板块资金、重点财经前瞻、股票事件、财报观察、强弱扫描、市场活跃指数和期权摘要。
...
美股行情列表页读取 `/api/product/symbols` 的 DB 查询结果，支持查询、板块、市值分层、事件/自选预设和排序参数。顶部全局搜索也使用同一接口按输入词查询股票。前端只渲染当前表格窗口和当前搜索结果，不再新增本地数据文件回退路径。
...
2. 增加只读 API，从 DB 查询股票详情、板块排行、重点财经前瞻。
