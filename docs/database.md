...
股票库列表页已经改为优先读取 `/api/product/symbols` 的 DB 查询结果，支持查询、板块、市值分层、事件/自选预设和排序参数。前端只渲染当前表格窗口，接口不可用时才回退到旧的本地聚合路径。
...
3. 前端从 JSON 逐步切到 API。股票库列表已经切换，后续继续迁移全局搜索、板块热力图和财经日历详情。
